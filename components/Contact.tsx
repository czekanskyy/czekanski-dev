...
const Contact: NextComponentType<NextPageContext, {}, Props> = ({ pageData }: Props) => {
  const form = useRef<HTMLFormElement>(null);

  // States for colors of the Submit button (used for animating sending)
  ...
  const submitForm = (e: FormEvent<HTMLFormElement>) => {
    // Func to reset form style back to initial
    ...
    // Clear form and blur focus
    e.preventDefault();
    e.currentTarget.reset();
    e.currentTarget.blur();

    // Change state to sending (yellow color)
    setBtnHoverColor('');
    ...
    emailjs.sendForm('service_46gto9n', 'template_czm98vm', form.current!, process.env.NEXT_PUBLIC_EMAILJS_KEY).then(
      result => {
        // IF sended show confirmation animation
        setTimeout(() => {
      ...
      error => {
        // IF NOT sended show error animation
  ...
  return (
    <div
      id='contact'
      className='h-screen flex flex-col items-center justify-center relative gap-y-6 sm-gap-y-8 md:gap-y-12 snap-center'
    >
      ...
      <div className='flex flex-col gap-y-3 sm:gap-y-5 md:gap-y-6 items-center'>
        ...
        <span className='flex gap-x-2 items-center'>
          ...
          <div className='flex gap-x-4 ml-4'>
            <a
              href={pageData?.resumeEN}
              download
              className='flex text-sm items-center gap-x-1 text-neutral-400'
            >
              <FaDownload />
              PL
            </a>
            <a
              href={pageData?.resumePL}
              download
              className='flex text-sm items-center gap-x-1 text-neutral-400'
            >
              <FaDownload />
      ...
      <form
        ref={form}
        onSubmit={submitForm}
        className='flex flex-col gap-y-2 w-full px-4 md:px-0 md:w-fit mx-auto'
      >
        ...
        <button
          type='submit'
          className={`transition-all ${btnColor} ${btnHoverColor} rounded-md p-2 sm:p-3 md:p-4 font-medium sm:text-lg md:text-xl`}
        >
          {btnText}
        </button>
      </form>
