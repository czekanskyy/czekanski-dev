...
const SkillItem = (props: Props) => {
  return (
    <motion.div
      ...
      className='w-20 h-20 md:w-28 md:h-28 xl:w-36 xl:h-36 border-2 border-neutral-500 flex items-center justify-center rounded-full p-2 sm:p-3 object-cover group overflow-hidden relative'
    >
      <motion.div className='filter group-hover:grayscale group-hover:blur-[2px] transition-all duration-300'>
        <Image src={urlFor(props.skill?.icon).url()} alt={props.skill?.title} width={144} height={144} />
      </motion.div>
